#ifndef lint
static char rcsid[] = "$Header: /usr/people/sam/tiff/tools/RCS/rgb2ycbcr.c,v 1.16 93/08/26 17:12:42 sam Exp $";
#endif
...
static void
cvtClump(unsigned char* op, unsigned long* raster, uint32 ch, uint32 cw, uint32 w)
{
	float Y, Cb = 0, Cr = 0;
	int j, k;
	/*
	...
	for (k = 0; k < ch; k++) {
		for (j = 0; j < cw; j++) {
			unsigned long RGB = (raster - k*w)[j];
 ...
 */
static void
cvtStrip(unsigned char* op, unsigned long* raster, uint32 nrows, uint32 width)
{
	uint32 x, y;
...
static int
cvtRaster(TIFF* tif, unsigned long* raster, uint32 width, uint32 height)
{
	uint32 y;
	tstrip_t strip = 0;
	tsize_t cc;
...
tiffcvt(TIFF* in, TIFF* out)
{
	...
	unsigned long* raster;		/* retrieve RGBA image */
	uint16 shortv;
	float floatv;
	char *stringv;
	uint32 longv;
	...
	raster = (unsigned long*)malloc(width * height * sizeof (long));
	if (raster == 0) {
