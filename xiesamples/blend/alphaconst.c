     ...
     caddr_t position;
{
  ...
  *(float *)data = newpercent;
  ...
  sprintf(snewpercent,"%.1f",newpercent);
     ...
     int *num_params;
{
  ...
  newpercent = (int)(fraction);
  if (newpercent != oldpercent) {
    oldpercent = newpercent;
  }
}

/* CreateAlphaConstScroller(w, appcontext)
** -------------------------
** Inside w (a form widget), creates:
**   1. A label 
**   2. A scrollbar for the user to choose the value (from 0 to 1.0)
**   3. A label with the current percentage displayed on it.
** The percentage starts at 1.0.
**
...
Widget
CreateAlphaConstScroller(w, appcontext, vert, horiz)
Widget w;
...
Widget horiz;
{
  ...
  ** can see it */
  ...
  label = XtCreateManagedWidget("Alpha",labelWidgetClass,w,
				labelargs,XtNumber(labelargs));

  percentargs[2].value = (XtArgVal) label;
  percentargs[3].value = (XtArgVal) vert;

  percent = XtCreateManagedWidget("1.0",labelWidgetClass,w,
  ...
  slider = XtCreateManagedWidget("AlphaSlider",scrollbarWidgetClass,w,
