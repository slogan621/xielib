...
char *argv[];
{
	...
	constant[ 0 ] = 1.0;
	while ((flag=getopt(argc,argv,"?c:d:i:o:"))!=EOF) {
		switch(flag) {
	...
	{
		printf( "Unable to read JPEG data from %s\n", img_file );
		usage( argv[ 0 ] );
	}
	if ( bands != 1 )
	{
	...
	/* the following is necessary so that the histogram window
           draws correctly */
	...
	XMoveWindow(display, histoWindow, w + 10, 0 );
...
typedef struct _opTableEnt
{
	char	*name;
	XieArithmeticOp value;
} OpTableEnt;
...
char	*str;
{
	...
	if ( !str )
		return( opTable[ 0 ].value );

	for ( i = 0; i < strlen( str ); i++ )
		str[ i ] = toupper( str[ i ] );

	for ( i = 0; i < sizeof( opTable ) / sizeof( OpTableEnt ); i++ )
	{
		if ( !strcmp( str, opTable[ i ].name ) )
			return( opTable[ i ].value );
	}
	return( opTable[ 0 ].value );
}
...
char *pgm;
{
	...
	printf("usage: %s [-c constant] [-d display] -i image -o operator\n", pgm);
