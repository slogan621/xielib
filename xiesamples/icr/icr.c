...
static XieRectangle rectangles[ NUM_RECTANGLES ];
...
char *argv[];
{
	...
	XiePhotoflo     flo;
	int             beSize, i, j, src, done, idx, floSize, bandMask;
	...
	while ((flag=getopt(argc,argv,"?d:i:m:"))!=EOF) {
		switch(flag) {
	...
	for ( i = 0; i < NUM_RECTANGLES; i++ )
	{
		...
		rectangles[ i ].x = rectangles[ i ].y = i * 64;
	}
	...
	XieFloImportClientROI(&flograph[idx],
		NUM_RECTANGLES		/* number of rectangles in ROI */
	);
	idx++;
	...
	domain.offset_y = 0;
	bandMask = 1;
	constant[ 0 ] = ( float ) max;

	XieFloArithmetic(&flograph[idx],
		src,		/* image source 1 */
		0,		/* image source 2 - none */
		&domain,	/* specifies the ROI as a process domain */
		constant,	/* second operand of arithmetic expression */
		xieValMax,	/* resulting pixel is max of src and constant */
	...
	PumpTheClientData( display, flo, 0, src + 1, rectangles, 
		sizeof( XieRectangle ) * NUM_RECTANGLES, 
		sizeof( XieRectangle ), 0, True );
...
char *pgm;
{
	printf("usage: %s [-d display] -m max -i image\n", pgm);
	exit(1);
}
