...
char *argv[];
{
	...
	while ((flag=getopt(argc,argv,"?d:i:t:"))!=EOF) {
		switch(flag) {
	...
	histoWindow = XCreateSimpleWindow(display, 
		DefaultRootWindow(display), 0, 0, MONWIDTH, MONHEIGHT, 
		1, 0, XWhitePixel( display, DefaultScreen( display ) ) );
	XSelectInput( display, histoWindow, ExposureMask );

        tgcv.foreground = XWhitePixel( display, DefaultScreen( display ) );
        tgcv.background = XBlackPixel( display, DefaultScreen( display ) );
    	gc = XCreateGC(display, histoWindow, GCForeground | GCBackground, 
		&tgcv);
	...
	levels[ 0 ] = 2; levels[ 1 ] = levels[ 2 ] = 0;
...
char *pgm;
{
	printf("usage: %s [-d display] [-t threshold] -i image\n", pgm);
	exit(1);
}
