...
char *argv[];
{
	...
	/* create a photomap resource to hold the image once it is 
	   transmitted by the client, so we don't have to re-transmit
	   each time the photoflo is re-executed */
	
	myPhotomap = XieCreatePhotomap( display );
	...
	XieFloExportPhotomap(&flograph[idx], 
		idx, 		/* source */
		myPhotomap, 	/* photomap to cache image to */
		xieValEncodeServerChoice, /* keep it canonical */
		( XiePointer ) NULL
	);
	idx++;
	...
	XieFloImportPhotomap( &flograph[idx],
		myPhotomap,	/* the photomap cache */
		False		/* don't notify client on decode */
	);
	idx++;	
	...
	XieRedefinePhotoflo( display, flo, flograph, floSize );
	...
	while (done == 0) {
        ...
        free( bytes );
	XFreeGC(display, gc);
	XieDestroyPhotomap( display, myPhotomap );
