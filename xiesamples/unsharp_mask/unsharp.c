...
char *argv[];
{
	...
	XiePointer	clipScaleParms = (XiePointer) NULL;
	...
	int             done, idx, src, floSize, floId, bandMask, size,
			screen, kernelSize, flag, beSize;
	...
	while ((flag=getopt(argc,argv,"?d:i:k:s:"))!=EOF) {
		switch(flag) {
	...
	constant[ 0 ] = constant[ 1 ] = constant[ 2 ] = scale;
...
char *pgm;
{
	printf("usage: %s [-d display] -i image -k kernel_file -s scale\n", pgm);
	exit(1);
}
