...
char *argv[];
{
	...
	char		deep, bands;
	short		w, h;
	XieArithmeticOp arithOp = xieValAdd;
	float		shift = 0.0, multiply = 1.0;
	...
	while ((flag=getopt(argc,argv,"?d:i:s:m:a:"))!=EOF) {
		switch(flag) {

		case 's':	shift = atof(optarg); 
				arithOp = xieValSub;
				break;

		case 'a':	shift = atof(optarg); 
				arithOp = xieValAdd;
				break;

		case 'm':	multiply = atof(optarg); 
				break;
...
char *pgm;
{
	printf("usage: %s [-d display] [-s constant | -a constant] [-m factor] -i image\n", pgm);
	exit(1);
}
