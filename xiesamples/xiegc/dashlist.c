     ...
     Widget w;
{
  ...
  dashes = (Widget *) malloc(BANDMASKLENGTH * sizeof(Widget));
  dashinfo = (int *) malloc(BANDMASKLENGTH * sizeof(int));

  /* make the label widget */
  label = XtCreateManagedWidget("BandMask",labelWidgetClass,w,
				labelargs,XtNumber(labelargs));

  dashargs[0].value = (XtArgVal) callbacklist;

  for (i=0;i<BANDMASKLENGTH;++i) {	/* go through all the buttons */
